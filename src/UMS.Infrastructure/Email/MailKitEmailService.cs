...
namespace UMS.Infrastructure.Email
{
    public sealed class MailKitEmailService(IOptions<EmailSettings> options) : IEmailService
    {
        ...
        public async Task SendStaffSetupAsync(
          string toEmail, string fullName, string role,
          string token, CancellationToken ct = default)
        {
            var setupUrl = $"{_settings.FrontendUrl}/setup-password?token={token}";

            await SendAsync(
                toEmail: toEmail,
                toName: fullName,
                subject: "You're Invited — Set Your Password",
                htmlBody: EmailTemplates.StaffSetup(fullName, role, setupUrl),
                ct: ct);
        }
